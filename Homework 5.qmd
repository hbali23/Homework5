...
model2_formula <- HeartDiseaseFactor ~ Age + RestingBP + Cholesterol + MaxHR + factor(Sex) + factor(ExerciseAngina) + factor(ChestPainType) + factor(RestingECG)
...
tree_formula <- HeartDiseaseFactor ~ Age + RestingBP + Cholesterol + MaxHR + factor(Sex) + factor(ExerciseAngina) + factor(ChestPainType) + factor(RestingECG)
...
##Rain Forest Model
```{r}
# Define tuning grid for random forest (mtry values)
mtry_values <- 1:ncol(heart_train)  # try all predictors
tune_grid3 <- expand.grid(mtry = mtry_values)
...
```{r}
# Define tuning grid for boosted tree (gbm parameters)
n_trees <- c(25, 50, 100, 200)
...
# Evaluate the boosted tree model on the test set
